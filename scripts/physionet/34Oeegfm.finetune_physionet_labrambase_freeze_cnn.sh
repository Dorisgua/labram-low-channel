...
DATASET="physionet"
CHANNEL_SUBSET="${CHANNEL_SUBSET:-physionet64}"
DATA_PATH="${DATA_PATH:-/inspire/hdd/project/sais-medical/public/share_medical/EEG/physionet/physionet.org/files/eegmmidb/processed_eegfmbench/processed/fs_200/motor_mv_img/finetune/1.0.0}"
# DATA_PATH="${DATA_PATH:-/inspire/ssd/tenant_predefaa-9a1b-4522-bb10-8850f313be13/global_user/7461-chenxinhe/physionet/physionet.org/files/eegmmidb/processed_eegfmbench/processed/fs_200/motor_mv_img/finetune/1.0.0}"
...
if [[ "${COMPLETION_SCOPE}" != "none" && ! -f "${CHANNEL_PROTOTYPE_PATH}" ]]; then
    echo "Missing PhysioNet channel prototype: ${CHANNEL_PROTOTYPE_PATH}"
    exit 1
fi
if [[ ! -f "${DATA_PATH}/dataset_info.json" ]]; then
    echo "Missing PhysioNet dataset metadata: ${DATA_PATH}/dataset_info.json"
    exit 1
fi
for split in train validation test; do
    if ! compgen -G "${DATA_PATH}/motor_mv_img-${split}-*.arrow" > /dev/null; then
        echo "Missing PhysioNet ${split} Arrow shards in ${DATA_PATH}"
        exit 1
    fi
done
