...
# Complete 3-seed Attention runs. Seed 0 has usually already been run by
# run_37_40_44_selected.sh, so this launcher defaults to seed 1 and seed 2.
# Override with ATTENTION_SEEDS="0 1 2" if you want to rerun all three seeds.
...
ATTENTION_O_SCRIPT="scripts/44Omeanpool.finetune_attention26_labrambase_full_finetune.sh"
...
BATCH_LOG="${BATCH_LOG_DIR}/run_44_attention_3seed_${BATCH_ID}.log"
...
for seed in "${ATTENTION_SEEDS[@]}"; do
    add_task "44O_attention26_full_seed${seed}" "${ATTENTION_O_SCRIPT}" "${seed}"
...
run_one() {
    ...
    local port=$((29800 + task_index))
    local run_tag="seed${seed}_attn$(printf '%02d' "${display_index}")"
    local -a launch_cmd=(
        env
        -u CHANNEL_SUBSET
        -u CLASSIFIER_MODE
...
if [[ "${parallel_status}" != "0" ]]; then
    echo "At least one Attention task failed; inspect ${BATCH_LOG} and per-run logs"
    exit "${parallel_status}"
fi

echo
echo "Completed all ${total_runs} Attention runs"
echo "Batch log: ${BATCH_LOG}"
