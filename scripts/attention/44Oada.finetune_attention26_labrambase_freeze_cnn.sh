...
# PreExp44 Attention baseline: 26 LaBraM-compatible EEG channels,
# BrainPro subject split 1-20/21-23/24-26, 4-second Attention/Rest windows,
...
DATA_PATH="${DATA_PATH:-/inspire/hdd/project/sais-medical/public/share_medical/EEG/Attention/processed_data_4s_200hz}"
# DATA_PATH="${DATA_PATH:-/inspire/ssd/tenant_predefaa-9a1b-4522-bb10-8850f313be13/global_user/7461-chenxinhe/Attention/processed_data_4s_200hz}"
CHANNEL_SUBSET="${CHANNEL_SUBSET:-attention26}"
...
EXP_GROUP="${EXP_GROUP:-preexp44_attention26_adabrain_real_brainpro_split}"
OUTPUT_ROOT="${OUTPUT_ROOT:-./outputs/attention/${OUTPUT_SCRIPT_NAME}}"
...
if [[ ! -f "${DATA_PATH}/manifest.json" ]]; then
    echo "Missing Attention manifest: ${DATA_PATH}/manifest.json"
...
if [[ -n "${CHANNEL_PROTOTYPE_PATH}" ]]; then
    if [[ ! -f "${CHANNEL_PROTOTYPE_PATH}" ]]; then
        echo "Missing channel prototype file: ${CHANNEL_PROTOTYPE_PATH}"
        exit 1
    fi
...

{
    ...
    printf '%q ' "${CMD[@]}"
    printf '\n\n'
    echo "Attention protocol: BrainPro split VP001-VP020 train, VP021-VP023 val, VP024-VP026 test; channel_subset=${CHANNEL_SUBSET}"
    echo "Task: binary Attention/Rest classification; 4s windows at 200 Hz"
    echo "Normalization: ${NORM_METHOD}"
    echo "Completion: scope=${COMPLETION_SCOPE}; prototype=${CHANNEL_PROTOTYPE_PATH:-none}"
    echo "Classifier: ${CLASSIFIER_MODE}; freeze_cnn=${FREEZE_CNN}; best_metric=${BEST_METRIC}"
