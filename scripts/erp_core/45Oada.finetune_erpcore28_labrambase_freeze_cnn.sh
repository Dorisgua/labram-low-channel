...
# ERP CORE PT baseline: CSLP-AE subject split, 12 classes excluding N170,
# LaBraM-compatible full EEG subset (28 channels; HEOG/VEOG excluded).
...
ADABRAIN_PRESERVE_BACKBONE_NO_WEIGHT_DECAY="${ADABRAIN_PRESERVE_BACKBONE_NO_WEIGHT_DECAY:-0}"
...
EXP_GROUP="${EXP_GROUP:-${SCRIPT_NAME%.sh}}"
OUTPUT_ROOT="${OUTPUT_ROOT:-./outputs/erpcore/${OUTPUT_SCRIPT_NAME}}"
...
if [[ ! -f "${DATA_PATH}" ]]; then
    echo "Missing ERP CORE PT file: ${DATA_PATH}"
    exit 1
fi
...

{
    ...
    printf '%q ' "${CMD[@]}"
    printf '\n\n'
    echo "ERP CORE protocol: CSLP-AE split; 12 classes excluding N170; channel_subset=${CHANNEL_SUBSET}"
    echo "Source PT: ${DATA_PATH}"
    echo "Sampling: source 256 Hz / 256 samples; loader target SAMPLING_RATE=${SAMPLING_RATE}"
    echo "Normalization: ${NORM_METHOD}"
    echo "Completion: scope=${COMPLETION_SCOPE}; pooling_scope=${POOLING_SCOPE}; prototype=${CHANNEL_PROTOTYPE_PATH:-none}"
    echo "Classifier: ${CLASSIFIER_MODE}; token_scope=${CLASSIFIER_TOKEN_SCOPE}; preserve_backbone_no_weight_decay=${ADABRAIN_PRESERVE_BACKBONE_NO_WEIGHT_DECAY}; freeze_cnn=${FREEZE_CNN}; best_metric=${BEST_METRIC}"
