...
本仓库当前环境应为 PyTorch 2.0.1、CUDA 11.8。不要直接升级 `torch`。

### CUDA out of memory

先用 `nvidia-smi` 确认同一张 GPU 没有其他训练任务。必要时降低 batch size：

```bash
BATCH_SIZE=16 UPDATE_FREQ=32 RUN_FOREGROUND=1 bash scripts/tuev/O/freeze_cnn.sh
```
...
```bash
MASTER_PORT=29511 bash scripts/tuev/O/freeze_cnn.sh
```

### Missing dataset、manifest 或 JSON

先用 `DRY_RUN=1` 查看最终 `--data_path`，再确认该路径存在。也可以临时覆盖：

```bash
DATA_PATH=/correct/dataset/path DRY_RUN=1 bash scripts/<dataset>/O/freeze_cnn.sh
```

BCI-IV-2a 的 `base.sh` 当前没有启用默认 `DATA_PATH`，需要显式指定 JSON
索引目录：
...
默认是后台运行，不一定表示失败。查看脚本打印的 `PID` 和 `Log`，或者使用：

```bash
RUN_FOREGROUND=1 bash scripts/tuev/O/freeze_cnn.sh
```

### 批量脚本注意事项

当前批量脚本包含 BCI-IV-2a，而它尚无默认 `DATA_PATH`，所以批量执行前应先
修复 BCI-IV-2a base。不要给跨数据集调度器统一设置一个 `DATA_PATH`，否则其他
数据集会继承错误路径。

不要将 API Key、访问令牌或密码写入本文档并提交到 Git。
