"""Generate SEED-V 62-channel prototypes from train-set CNN features."""
...
from Channels_definition import SEEDV_62_CHANNELS  # noqa: E402
from data_processor.seedv import SEEDVMotorEmotionDataset  # noqa: E402


SEEDV_ROOT = (
    "/inspire/ssd/tenant_predefaa-9a1b-4522-bb10-8850f313be13/"
    "global_user/7461-chenxinhe/SEED_V/SEED-V-labram"
)


def get_args():
    parser = argparse.ArgumentParser(
        "Generate SEED-V 62-channel CNN patch_embed prototypes"
    )
    parser.add_argument("--data_root", default=SEEDV_ROOT, type=str)
    parser.add_argument(
        "--finetune",
    ...
    parser.add_argument(
        "--output",
        default="docs/prototypes/01_seedv62_cnn_patch_embed_mean.pth",
        type=str,
    )
...
def main():
    ...
    # Deliberately instantiate only the train split so validation/test windows
    # cannot contribute to the prototypes.
    train_dataset = SEEDVMotorEmotionDataset(
        root=args.data_root,
        split="train",
        channel_names=SEEDV_62_CHANNELS,
    )
    ...
    ch_names = list(SEEDV_62_CHANNELS)
    ...
    for step, (samples, _) in enumerate(data_loader):
        ...
        # SEED-V preprocessing already stores samples in 0.1 mV units. This
        # matches DATASET_CONFIGS['SEEDV']['input_scale'] == 1.0.
    ...
    if num_patches is None or torch.any(channel_count == 0):
        raise RuntimeError("No SEED-V training features were accumulated")
    ...
    if not torch.isfinite(channel_prototypes).all():
        raise ValueError("Generated SEED-V prototypes contain NaN or Inf")
    ...
    payload = {
        ...
        "source_dataset": "SEED-V train split",
        ...
        "sample_scale": "1.0 (preprocessing already divided microvolts by 100)",
        "sampling_rate": 200,
        "patch_size": 200,
        "note": (
            "SEED-V 62-channel train-set prototypes from CNN patch embedding "
            "only; no validation/test samples and no Transformer blocks."
        ),
    }
    ...
    print(f"Saved SEED-V prototypes to {output_path}", flush=True)
