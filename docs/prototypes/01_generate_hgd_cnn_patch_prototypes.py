"""Generate HGD 78-channel prototypes from train CNN features."""
...
from Channels_definition import HGD_78_CHANNELS  # noqa: E402
from data_processor.hgd import prepare_HGD_official_dataset  # noqa: E402
...
def get_args():
    parser = argparse.ArgumentParser(
        "Generate HGD 78-channel CNN patch_embed prototypes"
    )
    parser.add_argument("--data_root", default=HGD_ROOT, type=str)
    parser.add_argument(
        "--finetune",
    ...
    parser.add_argument(
        "--output",
        default="docs/prototypes/01_hgd78_cnn_patch_embed_mean.pth",
        type=str,
    )
...
def main():
    ...
    train_dataset, _, _ = prepare_HGD_official_dataset(
        args.data_root,
        sampling_rate=args.sampling_rate,
        normalize_method=args.norm_method,
        channel_names=HGD_78_CHANNELS,
    ...
    if num_patches is None or torch.any(channel_count == 0):
        raise RuntimeError("No HGD training features were accumulated")
    ...
    if not torch.isfinite(channel_prototypes).all():
        raise ValueError("Generated HGD prototypes contain NaN or Inf")
    ...
    payload = {
        ...
        "source_dataset": "Schirrmeister2017-HGD official train split",
        ...
        "sample_scale": "1.0 (z_score normalized by HGD train split statistics)",
        "sampling_rate": args.sampling_rate,
        "patch_size": 200,
        "split_seed": args.split_seed,
        "note": (
            "HGD 78-channel train-set prototypes from CNN patch embedding only; "
    ...
    print(f"Saved HGD prototypes to {output_path}", flush=True)
