"""Generate EEGMAT 19-channel prototypes from train CNN features."""
...
from Channels_definition import EEGMAT_19_CHANNELS  # noqa: E402
from data_processor.eegmat import prepare_EEGMAT_cross_subject_dataset  # noqa: E402
...
def get_args():
    parser = argparse.ArgumentParser(
        "Generate EEGMAT 19-channel CNN patch_embed prototypes"
    )
    parser.add_argument("--data_root", default=EEGMAT_ROOT, type=str)
    parser.add_argument(
        "--finetune",
    ...
    parser.add_argument(
        "--output",
        default="docs/prototypes/01_eegmat19_cnn_patch_embed_mean.pth",
        type=str,
    )
...
def main():
    ...
    train_dataset, _, _ = prepare_EEGMAT_cross_subject_dataset(
        args.data_root,
        sampling_rate=args.sampling_rate,
        normalize_method=args.norm_method,
        channel_names=EEGMAT_19_CHANNELS,
    )
    ...
    ch_names = list(EEGMAT_19_CHANNELS)
    ...
    if num_patches is None or torch.any(channel_count == 0):
        raise RuntimeError("No EEGMAT training features were accumulated")
    ...
    if not torch.isfinite(channel_prototypes).all():
        raise ValueError("Generated EEGMAT prototypes contain NaN or Inf")
    ...
    payload = {
        ...
        "source_dataset": "EEGMAT cross-subject train split",
        ...
        "sample_scale": "1.0 (z_score normalized by EEGMAT train split statistics)",
        "sampling_rate": args.sampling_rate,
        "normalize_method": args.norm_method,
        "patch_size": 200,
        "note": (
            "EEGMAT 19-channel train-set prototypes from CNN patch embedding only; "
    ...
    print(f"Saved EEGMAT prototypes to {output_path}", flush=True)
