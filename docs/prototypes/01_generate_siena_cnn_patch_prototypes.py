"""Generate Siena 29-channel prototypes from train CNN features only."""
...
from Channels_definition import SIENA_29_CHANNELS  # noqa: E402
from data_processor.siena import prepare_Siena_cross_subject_dataset  # noqa: E402
...
def get_args():
    parser = argparse.ArgumentParser(
        "Generate Siena 29-channel CNN patch_embed prototypes"
    )
    parser.add_argument("--data_root", default=SIENA_ROOT, type=str)
    parser.add_argument(
        "--finetune",
    ...
    parser.add_argument(
        "--output",
        default="docs/prototypes/01_siena29_cnn_patch_embed_mean.pth",
        type=str,
    )
...
def main():
    ...
    train_dataset, _, _ = prepare_Siena_cross_subject_dataset(
        args.data_root,
        sampling_rate=args.sampling_rate,
        normalize_method=args.norm_method,
        channel_names=SIENA_29_CHANNELS,
    )
    ...
    ch_names = list(SIENA_29_CHANNELS)
    ...
    if num_patches is None or torch.any(channel_count == 0):
        raise RuntimeError("No Siena training features were accumulated")
    ...
    if not torch.isfinite(channel_prototypes).all():
        raise ValueError("Generated Siena prototypes contain NaN or Inf")
    ...
    payload = {
        ...
        "source_dataset": "Siena cross-subject train split",
        ...
        "sample_scale": "1.0 (z_score normalized by Siena train split statistics)",
        "sampling_rate": args.sampling_rate,
        "normalize_method": args.norm_method,
        "patch_size": 200,
        "note": (
            "Siena 29-channel train-set prototypes from CNN patch embedding only; "
    ...
    print(f"Saved Siena prototypes to {output_path}", flush=True)
