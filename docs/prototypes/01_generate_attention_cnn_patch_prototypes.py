"""Generate Attention 26-channel prototypes from train CNN features only."""
...
from Channels_definition import ATTENTION_26_CHANNELS  # noqa: E402
from data_processor.attention import prepare_Attention_cross_subject_dataset  # noqa: E402
...
def get_args():
    parser = argparse.ArgumentParser(
        "Generate Attention 26-channel CNN patch_embed prototypes"
    )
    parser.add_argument("--data_root", default=ATTENTION_ROOT, type=str)
    parser.add_argument(
        "--finetune",
    ...
    parser.add_argument(
        "--output",
        default="docs/prototypes/01_attention26_cnn_patch_embed_mean.pth",
        type=str,
    )
...
def main():
    ...
    train_dataset, _, _ = prepare_Attention_cross_subject_dataset(
        args.data_root,
        sampling_rate=args.sampling_rate,
        normalize_method=args.norm_method,
        channel_names=ATTENTION_26_CHANNELS,
    )
    ...
    ch_names = list(ATTENTION_26_CHANNELS)
    ...
    if num_patches is None or torch.any(channel_count == 0):
        raise RuntimeError("No Attention training features were accumulated")
    ...
    if not torch.isfinite(channel_prototypes).all():
        raise ValueError("Generated Attention prototypes contain NaN or Inf")
    ...
    payload = {
        ...
        "source_dataset": "Attention cross-subject train split",
        ...
        "sample_scale": "1.0 (z_score normalized by Attention train split statistics)",
        "sampling_rate": args.sampling_rate,
        "normalize_method": args.norm_method,
        "patch_size": 200,
        "note": (
            "Attention 26-channel train-set prototypes from CNN patch embedding only; "
            "no validation/test samples and no Transformer blocks."
        ),
    }
    ...
    print(f"Saved Attention prototypes to {output_path}", flush=True)
