"""Generate SEED 62-channel prototypes from train-split CNN features."""
...
from Channels_definition import SEED_62_CHANNELS  # noqa: E402
from data_processor.seed import prepare_SEED_cross_subject_dataset  # noqa: E402


SEED_ROOT = (
    "/inspire/ssd/tenant_predefaa-9a1b-4522-bb10-8850f313be13/"
    "global_user/7461-chenxinhe/SEED/processed_data"
)


def get_args():
    parser = argparse.ArgumentParser(
        "Generate SEED 62-channel CNN patch_embed prototypes"
    )
    parser.add_argument("--data_root", default=SEED_ROOT, type=str)
    parser.add_argument(
        "--finetune",
    ...
    parser.add_argument(
        "--output",
        default="docs/prototypes/01_seed62_cnn_patch_embed_mean.pth",
        type=str,
    )
...
def main():
    ...
    # Only the cross-subject training split contributes to prototypes.
    train_dataset, _, _ = prepare_SEED_cross_subject_dataset(
        args.data_root,
    ...
    for step, (samples, _) in enumerate(data_loader):
        ...
        # Match DATASET_CONFIGS['SEED']['input_scale'] during fine-tuning.
    ...
    if num_patches is None or torch.any(channel_count == 0):
        raise RuntimeError("No SEED training features were accumulated")
    ...
    if not torch.isfinite(channel_prototypes).all():
        raise ValueError("Generated SEED prototypes contain NaN or Inf")
    ...
    payload = {
        ...
        "source_dataset": "SEED cross-subject train split (subjects 1-12 train partition)",
        "prototype_type": "cnn_patch_embed_mean",
        "completion_scope": "seed23_with_seed62",
        ...
        "sample_scale": "0.01 (microvolts, matching SEED fine-tuning)",
        "sampling_rate": 200,
        "patch_size": 200,
        "note": (
            "SEED 62-channel train-split prototypes from CNN patch embedding "
    ...
    print(f"Saved SEED prototypes to {output_path}", flush=True)
